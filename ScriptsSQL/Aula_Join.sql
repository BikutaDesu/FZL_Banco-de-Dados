...
--SQL 2
SELECT SUBSTRING(am.ra_aluno, 1, 9) + '-' + SUBSTRING(am.ra_aluno, 10, 1) AS ra, 
		CASE WHEN (LEN(a.nome) >= 30) THEN
			SUBSTRING(a.nome, 1, 29)+'.'
		ELSE 
			a.nome
		END AS nome
FROM materias m INNER JOIN alunomateria am
...
FROM tabela1 LEFT OU RIGHT OUTER JOIN tabela2
ON tabela1.pk = tabela2.fk
WHERE tabela2.fk IS NULL
		AND condições ...
*/
